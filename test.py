...
for file_index, filename in enumerate(os.listdir(main_folder+move_type)):

    dataset = pd.read_csv(main_folder+move_type+filename, sep=";", decimal=",", header=None)
    ...
    plt.tight_layout()
    plt.savefig("{}_results.png".format(filename.split('.')[0]))
    print("{}_results.png saved...".format(filename.split('.')[0]))
    print("WL array:\n",WL_arr)
    print("ZC array:\n",ZC_arr)
    print("VAR array:\n",VAR_arr)
    print("MAV array:\n",MAV_arr)
